...
class Api0Logger extends Interceptor {
  ...
  void onError(DioError err, ErrorInterceptorHandler handler) {
    if (error) {
      if (err.type == DioErrorType.response) {
        ...
        _printBoxed(
            header:
                'Api0Error ║ Status: ${err.response?.statusCode} ${err.response?.statusMessage}',
            text: uri.toString());
        ...
        _printBoxed(header: 'Api0Error ║ ${err.type}', text: err.message);
      }
    }
  ...
  void printAPI0Response(API0Response? response) {
    try {
      if (response != null) {
        if (response.data is Map) {
          logPrint('╔╣ Body');
